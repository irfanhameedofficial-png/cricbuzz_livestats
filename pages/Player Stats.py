...
if selected_player:
    ...
    with tab1:
        ...
        headers = {
            "x-rapidapi-key": "5230173082mshf579aeff7266f90p10f970jsn0d8bf046224f",
            "x-rapidapi-host": "cricbuzz-cricket.p.rapidapi.com"
        }
        #Remove comment line below for live execution--------------------------------------
        ...
        st.subheader(f"ℹ️ {searched_player_info['name']} ({searched_player_info['nickName']})")
        st.write(f"**Role:** {searched_player_info['role']}")
        st.write(f"**Batting:** {searched_player_info['bat']}")
        st.write(f"**Bowling:** {searched_player_info['bowl']}")

        # Player Details
        st.markdown(f"""
        **🆔 ID:** {searched_player_info['id']}  
        **🌍 International Team:** {searched_player_info['intlTeam']}  
        **🎂 Date of Birth:** {searched_player_info['DoB']}  
        **🏟️ Birthplace:** {searched_player_info['birthPlace']}  
        """)

        st.markdown("### 🏆 Teams Played For")
        st.write(searched_player_info['teams'])
